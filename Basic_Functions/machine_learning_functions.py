...
# =============================================================================
    ...
    for i in range(0,E_Regulation.size):
        ...
        val = val**(log_scale_activity)
        x[0][0][i] = np.log(1.0 + np.exp(range_of_activity_scale)*val)
        count+=1
    y_pred = nn_model(x)
    val=y_pred
...
def reward_value(v_log_counts_future, v_log_counts_old,\
                 ...
                 KQ_f_old, KQ_r_old, E_Regulation_old,\
    ...
    #epr_old = entropy_production_rate(KQ_f_old, KQ_r_old, E_Regulation_old)
    ...
    if (reward_s > 0.0):
        #diff_epr = epr_future - epr_old #rewarded if epr increases. 
        #epr = np.exp(diff_epr)
        
        #diff_loss = np.sum(val_old - val_future)
        #loss = np.exp(diff_loss) #positive reward if objective loss decreases
        
        final_reward = reward_s#epr * loss
        
       
        #this upper one works really well, but it makes you take as many short steps as possible. 
        #but is the data changing over time in this case? 
        
        #if negative (-0.01) -> take fastest path
        #if positive (0.01) -> take slowest path
        
    if ((delta_s_next<=0.0).all()):
        epr_future = entropy_production_rate(KQ_f_new, KQ_r_new, E_Regulation_new)
    
        final_reward = 1.0 * epr_future
        #breakpoint()
...
def sarsa_lambda( threshold,nn_model,loss_fn, optimizer, step_size, n_back_step, theta_linear, v_log_counts_static, state_sample, epsilon_greedy):
    ...
    #These variables are modified in the next while loop.
    path_state_sample = state_sample.copy() 
    path_state_sample_previous = state_sample.copy()
    ...
    KQ_f_path_previous = KQ_f_init.copy()
    
    KQ_r_path = KQ_r_init.copy()
    KQ_r_path_previous = KQ_r_init.copy()
    ...
    for t in range(0,end_of_path):
        ...
        if (t < end_of_path):
            ...
            action_value = reward_value(v_log_counts_path, v_log_counts_path_previous, 
                                        KQ_f_path, KQ_r_path, path_state_sample,\
                                        KQ_f_path_previous,KQ_r_path_previous,path_state_sample_previous,\
            ...
            path_state_sample_previous = path_state_sample.copy()
            KQ_f_path_previous = KQ_f_path.copy()
            KQ_r_path_previous = KQ_r_path.copy()
        ...
        if (t >=0):
            ...
            previous_estimate = state_value(nn_model,theta_linear, delta_S_matrix[:,t],\
                                           delta_S_metab_matrix[:,t], KQ_f_matrix[:,t], KQ_r_matrix[:,t], \
                                           states_matrix[:,t], v_log_counts_matrix[:,t])
            
            current_estimate = state_value(nn_model,theta_linear, delta_S_matrix[:,t+1],\
                                           delta_S_metab_matrix[:,t+1], KQ_f_matrix[:,t+1], KQ_r_matrix[:,t+1], \
                                           states_matrix[:,t+1], v_log_counts_matrix[:,t+1])
                    
            

            delta = value_path[t+1] + gamma * current_estimate - previous_estimate
            
            #z = gamma * lambda * z + (1 - step)
            ...
            delta_S_tau = delta_S_matrix[:, tau].copy()
                    
            v_log_counts_tau = v_log_counts_matrix[:, tau].copy()
            #alt_value_tau = alt_value_matrix[:, tau].copy()
            delta_S_metab_tau = delta_S_metab_matrix[:, tau].copy()
                
            value_tau = state_value(nn_model,theta_linear, delta_S_tau, delta_S_metab_tau, KQ_f_tau, KQ_r_tau, state_tau, v_log_counts_tau)
...
# =============================================================================
    ...
    print(act_path[0:t+1])
    return [sum_reward_episode, average_loss_episode]


def sarsa_n( nn_model,loss_fn, optimizer, scheduler, step_size, n_back_step, theta_linear, v_log_counts_static, state_sample, epsilon_greedy):
    #First we sample m states from E
    #First args input is epsilon
    #Second args input is a matrix of states to fit to.
...
#             #y_pred = nn_model(x)
#                 
#             #loss_previous = loss_fn(y_pred, y)
# 
#             #print("initial Loss")
#             #print(loss_previous.item())
#             
#             def closure():
...
# =============================================================================
            ...
            loss = loss_fn(y_pred, y) #/y??? to normalize
...
def policy_function(nn_model,state, theta_linear, v_log_counts_path, *args ):
    ...
    for act in range(0,reaction_size_to_regulate):
        ...
        current_reward = reward_value(new_v_log_counts, v_log_counts, 
                                      KQ_f_new, KQ_r_new, trial_state_sample,\
                                      KQ_f, KQ_r, state,\
                                      delta_S_new, delta_S)
